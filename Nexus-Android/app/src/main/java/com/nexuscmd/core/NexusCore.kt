 ...
 * 支持为不同的资源包同时创建多个内核实例
 */
class NexusCore private constructor(
    ...
    init {
        ...
        if (pointer == 0L) {
            throw RuntimeException("fail to init Nexus Core: $path")
        }
    }
    ...
    companion object {
        init {
            // 加载c++内核
            System.loadLibrary("NexusAndroid")
        }
         ...
         * @return 软件内核
         */
        fun fromAssets(assetManager: AssetManager, path: String): NexusCore {
            return NexusCore(assetManager, path)
        }
         ...
         * @return 软件内核
         */
        fun fromFile(path: String): NexusCore {
            return NexusCore(null, path)
        }
