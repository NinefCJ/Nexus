...
package com.nexuscmd.android.service
...
class Old2NewIMEService : InputMethodService() {
    ...
    private var theme by mutableStateOf(NexusTheme.Theme.Light)
    ...
    override fun onCreate() {
        ...
        composeLifecycleOwner.lifecycleScope.launch {
            settingsDataStore.themeId().collect { themeId ->
                theme = when (themeId) {
                    "MODE_NIGHT_NO" -> NexusTheme.Theme.Light
                    "MODE_NIGHT_YES" -> NexusTheme.Theme.Dark
                    else -> if (isSystemDarkMode) NexusTheme.Theme.Dark else NexusTheme.Theme.Light
                }
                val newNightMode =
                    if (theme == NexusTheme.Theme.Dark) Configuration.UI_MODE_NIGHT_YES else Configuration.UI_MODE_NIGHT_NO
    ...
    override fun onWindowShown() {
        ...
        // 转换为新版命令
        newCommand = NexusCore.old2new(this, oldCommand)
