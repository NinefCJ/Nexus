...
/**
 * NexusCore 是整个软件的命令解析中枢，但它依赖 JNI（libNexusAndroid.so）。
...
class NexusCoreInstrumentedTest {
     ...
     */
    private fun withCore(block: (NexusCore) -> Unit) {
        val ctx = InstrumentationRegistry.getInstrumentation().targetContext
        val core = try {
            NexusCore.fromAssets(ctx.assets, cpackPath)
        } catch (e: Throwable) {
            // 当前 ABI 不匹配或资源缺失时不算测试失败——
            // 这种环境下根本跑不起来这个测试，硬挂只会污染 CI 信号
            Assume.assumeNoException("跳过：NexusCore 在当前环境无法初始化", e)
            return
        }
        try {
            block(core)
