...
template<class T, bool isConvertEndian>
void testNode(Nexus::CPack &cpack, const std::function<T()> &getInstance) {
    std::ostringstream oss;
    T t1 = getInstance();
    serialization::Codec<T>::template to_binary<isConvertEndian>(oss, t1);
    std::istringstream iss(oss.str());
...
template<class T>
void testNode(Nexus::CPack &cpack,
...
TEST(BinaryUtilTest, Manifest) {
    ...
    test<Nexus::Manifest>({getInstance1, getInstance2, getInstance3, getInstance4});
}

TEST(BinaryUtilTest, NormalId) {
    auto getInstance1 = []() {
        return Nexus::NormalId::make(u"name", u"description");
    };
    auto getInstance2 = []() {
        return Nexus::NormalId::make(u"name", u"");
    };
    auto getInstance3 = []() {
        return Nexus::NormalId::make(u"name", std::nullopt);
    };
    test<std::shared_ptr<Nexus::NormalId>>({getInstance1, getInstance2, getInstance3});
}
...
TEST(BinaryUtilTest, NamespaceId) {
    ...
    std::vector<std::function<Nexus::NamespaceId()>> getInstance;
    for (const auto &item: serialization::find_array_member_or_throw(j, "content")) {
        Nexus::NamespaceId namespaceId;
        serialization::Codec<Nexus::NamespaceId>::from_json(item, namespaceId);
        getInstance.emplace_back([namespaceId]() { return namespaceId; });
    }
    test<Nexus::NamespaceId>(getInstance);
}
...
TEST(BinaryUtilTest, ItemId) {
    ...
    std::vector<std::function<Nexus::ItemId()>> getInstance;
    for (const auto &item: serialization::find_array_member_or_throw(j, "content")) {
        getInstance.emplace_back([&item]() {
            Nexus::ItemId itemId;
            serialization::Codec<Nexus::ItemId>::from_json(item, itemId);
            return itemId;
        });
    }
    test<Nexus::ItemId>(getInstance);
}

TEST(BinaryUtilTest, Property) {
    auto getInstance1 = []() {
        Nexus::Property aProperty;
        aProperty.name = u"name1";
        aProperty.type = Nexus::PropertyType::BOOLEAN;
        aProperty.defaultValue.boolean = true;
        aProperty.valid = std::vector<Nexus::PropertyValue>(2);
    ...
    auto getInstance2 = []() {
        ...
        aProperty.type = Nexus::PropertyType::INTEGER;
        aProperty.defaultValue.integer = 0;
        aProperty.valid = std::vector<Nexus::PropertyValue>(3);
    ...
    auto getInstance3 = []() {
        ...
        aProperty.type = Nexus::PropertyType::STRING;
    ...
    test<Nexus::Property>({getInstance1, getInstance2, getInstance3});
}
...
TEST(BinaryUtilTest, BlockId) {
    ...
    Nexus::BlockIds blockIds;
    serialization::Codec<Nexus::BlockIds>::from_json(serialization::find_member_or_throw(j, "content"), blockIds);
    test<Nexus::BlockIds>([&blockIds]() { return blockIds; });
}

TEST(BinaryUtilTest, PerCPackNormalIds) {

    std::unique_ptr<Nexus::CPack> cpack;
    try {
        std::filesystem::path resourceDir(RESOURCE_DIR);
        cpack = Nexus::CPack::createByDirectory(
                std::filesystem::path(resourceDir / "resources" / "beta" / "vanilla"));
    } catch (const std::exception &e) {
        Nexus::Profile::printAndClear(e);
        exit(-1);
    }
    std::vector<std::function<
            std::shared_ptr<std::vector<std::shared_ptr<Nexus::NormalId>>>()>>
            getInstance;
    for (const auto &item: cpack->normalIds) {
        const auto &ids = item.second;
        getInstance.emplace_back([&ids]() { return ids; });
    }
    test<std::shared_ptr<std::vector<std::shared_ptr<Nexus::NormalId>>>>(getInstance);
}
...
TEST(BinaryUtilTest, CPackNormalIds) {
    ...
    try {
        std::filesystem::path resourceDir(RESOURCE_DIR);
        cpack = Nexus::CPack::createByDirectory(resourceDir / "resources" /
                                                  "beta" / "vanilla");
    } catch (const std::exception &e) {
        Nexus::Profile::printAndClear(e);
        exit(-1);
    }
    test<std::unordered_map<std::string, std::shared_ptr<std::vector<std::shared_ptr<Nexus::NormalId>>>>>(
...
TEST(BinaryUtilTest, CPackNamespaceId) {
    ...
    test<std::unordered_map<
            std::string,
            std::shared_ptr<std::vector<std::shared_ptr<Nexus::NamespaceId>>>>>(
...
TEST(BinaryUtilTest, NodeJsonBoolean) {
    ...
    Nexus::Node::NodeJsonBoolean node("ID", u"description", u"descriptionTrue", u"descriptionFalse");
    testNode<Nexus::Node::NodeJsonBoolean>(*cpack, [&node]() { return node; });
}
...
TEST(BinaryUtilTest, NodeJsonInteger) {
    ...
    testNode<Nexus::Node::NodeJsonInteger>(
            *cpack,
            {
                    []() {
                        Nexus::Node::NodeJsonInteger node;
                        node.id = "ID";
...
TEST(BinaryUtilTest, NodeJsonFloat) {
    ...
    testNode<Nexus::Node::NodeJsonFloat>(
            *cpack,
            {
                    []() {
                        Nexus::Node::NodeJsonFloat node;
                        node.id = "ID";
                        node.description = u"description";
                        node.isMustAfterSpace = false;
                        node.min = 0.0F;
...
TEST(BinaryUtilTest, NodeJsonNull) {
    ...
    try {
        std::filesystem::path resourceDir(RESOURCE_DIR);
        cpack = Nexus::CPack::createByDirectory(resourceDir / "resources" / "beta" / "vanilla");
    } catch (const std::exception &e) {
        Nexus::Profile::printAndClear(e);
        exit(-1);
    }
    testNode<Nexus::Node::NodeJsonNull>(
            *cpack, []() { return Nexus::Node::NodeJsonNull{"ID", u"description"}; });
}
