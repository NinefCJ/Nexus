...
int main() {
    ...
    // return 0;
    bool isSuccess = true;
    isSuccess = outputFile(Nexus::Test::writeSingleJson, "json") && isSuccess;
    isSuccess = outputFile(Nexus::Test::writeBinary, "cpack") && isSuccess;
...
[[maybe_unused]] void testDir() {
    ...
    Nexus::Test::testDir(resourceDir / "resources" / "beta" / "vanilla",
...
[[maybe_unused]] void testBin() {
    ...
    for (const auto &cpackPath: std::filesystem::directory_iterator(resourceDir / "generated" / "cpack")) {
        ...
        if (fileName.find("beta-experiment-") != std::string::npos) {
            Nexus::Test::testBin(cpackPath, resourceDir / "test" / "test.txt", true);
            break;
        }
    }
}
...
bool outputOld2New() {
    ...
    Nexus::Old2New::BlockFixData blockFixData = Nexus::Old2New::blockFixDataFromJson(serialization::get_json_from_file(input));
...
namespace Nexus::Test {
    ...
    [[maybe_unused]] void testDir(const std::filesystem::path &cpackPath, const std::filesystem::path &testFilePath, bool isTestTime) {
        ...
        fin.close();
        Nexus::Test::testDir(cpackPath, commands, isTestTime);
        ...
        //     }
        // }
        // Nexus::Test::test2(cpackPath, commands1, 10);
    }
    ...
    [[maybe_unused]] void testBin(const std::filesystem::path &cpackPath, const std::filesystem::path &testFilePath, bool isTestTime) {
        ...
        fin.close();
        Nexus::Test::testBin(cpackPath, commands, isTestTime);
    }
