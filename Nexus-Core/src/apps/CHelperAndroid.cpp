...
Java_com_nexuscmd_core_NexusCore_create0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jobject assetManager, jstring cpack_path) {
    if (cpack_path == nullptr) {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_create0 when cpack_path is null");
        return reinterpret_cast<jlong>(nullptr);
    }
    try {
        std::string cpackPath = jstring2string(env, cpack_path);
        if (assetManager == nullptr) [[unlikely]] {
            Nexus::CHelperCore *core = Nexus::CHelperCore::createByBinary(cpackPath);
            ...
            Nexus::CHelperCore *core = Nexus::CHelperCore::create([&iss]() {
                return Nexus::CPack::createByBinary(iss);
            });
            delete[] buffer;
            return reinterpret_cast<jlong>(core);
        }
...
Java_com_nexuscmd_core_NexusCore_release0(
        [[maybe_unused]] JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer) {
    delete reinterpret_cast<Nexus::CHelperCore *>(pointer);
}

extern "C" [[maybe_unused]] JNIEXPORT void JNICALL
Java_com_nexuscmd_core_NexusCore_onTextChanged0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer, jstring text, jint index) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_onTextChanged0 when core is nullptr");
        return;
    }
...
extern "C" [[maybe_unused]] JNIEXPORT void JNICALL
Java_com_nexuscmd_core_NexusCore_onSelectionChanged0(
        [[maybe_unused]] JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer, jint index) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_onSelectionChanged0 when core is nullptr");
        return;
    }
...
Java_com_nexuscmd_core_NexusCore_getParamHint0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getParamHint0 when core is nullptr");
        return nullptr;
    }
...
Java_com_nexuscmd_core_NexusCore_getErrorReasons0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    jclass errorReasonClass = env->FindClass("yancey/chelper/core/ErrorReason");
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getErrorReasons0 when core is nullptr");
    ...
    for (size_t i = 0; i < errorReasons.size(); ++i) {
        const Nexus::ErrorReason &item = *errorReasons[i];
...
extern "C" [[maybe_unused]] JNIEXPORT jlong JNICALL
Java_com_nexuscmd_core_NexusCore_getSuggestionsSize0(
        [[maybe_unused]] JNIEnv *env, [[maybe_unused]] jint thiz, jlong pointer) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getSuggestionsSize0 when core is nullptr");
        return 0;
    }
...
Java_com_nexuscmd_core_NexusCore_getSuggestion0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer, jint which) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getSuggestion0 when core is nullptr");
        return nullptr;
    }
    if (which < 0) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getSuggestion0 when which < 0");
        return nullptr;
    }
    auto suggestions = core->getSuggestions();
    if (static_cast<jint>(suggestions->size()) <= which) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getSuggestion0 when suggestions->size() <= which");
        return nullptr;
    }
    Nexus::AutoSuggestion::Suggestion suggestion = suggestions->at(which);
...
extern "C" [[maybe_unused]] JNIEXPORT jobject JNICALL
Java_com_nexuscmd_core_NexusCore_getSuggestions0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    jclass suggestionClass = env->FindClass("yancey/chelper/core/Suggestion");
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getSuggestions0 when core is nullptr");
        return env->NewObjectArray(0, suggestionClass, nullptr);
    }
    const std::vector<Nexus::AutoSuggestion::Suggestion> &suggestions = *core->getSuggestions();
    jobjectArray result = env->NewObjectArray(static_cast<jsize>(suggestions.size()), suggestionClass, nullptr);
    for (size_t i = 0; i < suggestions.size(); ++i) {
        const Nexus::AutoSuggestion::Suggestion &item = suggestions[i];
        jobject javaSuggestion = env->AllocObject(suggestionClass);
        env->SetObjectField(javaSuggestion,
                            env->GetFieldID(suggestionClass, "name", "Ljava/lang/String;"),
...
extern "C" [[maybe_unused]] JNIEXPORT jstring JNICALL
Java_com_nexuscmd_core_NexusCore_getStructure0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getStructure0 when core is nullptr");
        return nullptr;
    }
...
extern "C" [[maybe_unused]] JNIEXPORT jobject JNICALL
Java_com_nexuscmd_core_NexusCore_onSuggestionClick0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer, jint which) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_onSuggestionClick0 when core is nullptr");
        return nullptr;
    }
...
Java_com_nexuscmd_core_NexusCore_getColors0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jlong pointer) {
    auto *core = reinterpret_cast<Nexus::CHelperCore *>(pointer);
    if (core == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_getColors0 when core is nullptr");
        return nullptr;
    }
...
Nexus::Old2New::BlockFixData blockFixData0;

extern "C" [[maybe_unused]] JNIEXPORT jboolean JNICALL
Java_com_nexuscmd_core_NexusCore_old2newInit0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jobject assetManager, jstring blockFixDataPath) {
    if (blockFixDataPath == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_old2newInit0 when blockFixDataPath is nullptr");
        return false;
    }
...
extern "C" [[maybe_unused]] JNIEXPORT jstring JNICALL
Java_com_nexuscmd_core_NexusCore_old2new0(
        JNIEnv *env, [[maybe_unused]] jobject thiz, jstring old) {
    if (old == nullptr) [[unlikely]] {
        SPDLOG_WARN("call Java_com_nexuscmd_core_NexusCore_old2new0 when old is nullptr");
        return nullptr;
    }
    return u16string2jstring(env, Nexus::CHelperCore::old2new(blockFixData0, jstring2u16string(env, old)));
}
