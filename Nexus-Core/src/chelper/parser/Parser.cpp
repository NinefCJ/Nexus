...
namespace Nexus::Parser {
    ...
    struct Parser<Node::NodeJsonList> {
        static ASTNode getASTNode(const Node::NodeJsonList &node, TokenReader &tokenReader) {
            if (!node.nodeList.has_value()) [[unlikely]] {
                return parseByChildNode(node, tokenReader, Nexus::Node::NodeJsonList::nodeAllList, ASTNodeId::NODE_JSON_ALL_LIST);
            }
            tokenReader.push();
            ...
            tokenReader.restore();
            tokenReader.push();
            ASTNode result2 = parseByChildNode(node, tokenReader, Nexus::Node::NodeJsonList::nodeAllList, ASTNodeId::NODE_JSON_ALL_LIST);
    ...
    struct Parser<Node::NodeBlock> {
        static ASTNode getASTNode(const Node::NodeBlock &node, TokenReader &tokenReader) {
            ...
            tokenReader.push();
            ASTNode blockStateLeftBracket = parse(Nexus::Node::NodeBlock::nodeBlockStateLeftBracket, tokenReader);
            tokenReader.restore();
    ...
    struct Parser<Node::NodeItem> {
        static ASTNode getASTNode(const Node::NodeItem &node, TokenReader &tokenReader) {
            ...
            Node::NodeWithType nodeData = currentItem == nullptr ? Nexus::Node::NodeItem::nodeAllData : std::static_pointer_cast<ItemId>(currentItem)->getNode();
