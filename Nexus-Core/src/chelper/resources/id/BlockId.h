...
template<>
struct serialization::Codec<Nexus::PropertyValue> {

    using Type = Nexus::PropertyValue;
    ...
    template<class JsonValueType>
    static Nexus::PropertyType::PropertyType
...
CODEC_ENUM(Nexus::PropertyType::PropertyType, uint8_t);

CODEC_REGISTER_JSON_KEY(Nexus::Property, name, defaultValue, valid);

template<>
struct serialization::Codec<Nexus::Property> : BaseCodec<Nexus::Property> {

    using Type = Nexus::Property;
    ...
    static void to_json(typename JsonValueType::AllocatorType &allocator,
                        ...
                        const Type &t) {
        jsonValue.SetObject();
        Codec<decltype(t.name)>::template to_json_member<JsonValueType>(allocator, jsonValue, details::JsonKey<Nexus::Property, typename JsonValueType::Ch>::name_(), t.name);
        Codec<decltype(t.defaultValue)>::template to_json_member<JsonValueType>(allocator, jsonValue, details::JsonKey<Nexus::Property, typename JsonValueType::Ch>::defaultValue_(), t.defaultValue, t.type);
        if (t.valid.has_value()) {
            ...
            for (const auto &item: t.valid.value()) {
                ...
                Codec<Nexus::PropertyValue>::template to_json<typename JsonValueType::ValueType>(allocator, perValid, item, t.type);
                valid.PushBack(std::move(perValid), allocator);
            }
            jsonValue.AddMember(JsonValueType(details::JsonKey<Nexus::Property, typename JsonValueType::Ch>::valid_(), allocator), std::move(valid), allocator);
        }
    }
    ...
    static void from_json(const JsonValueType &jsonValue,
        ...
        t.release();
        Codec<decltype(t.name)>::template from_json_member<JsonValueType>(jsonValue, details::JsonKey<Nexus::Property, typename JsonValueType::Ch>::name_(), t.name);
        t.type = Codec<decltype(t.defaultValue)>::template from_json_member<JsonValueType>(jsonValue, details::JsonKey<Nexus::Property, typename JsonValueType::Ch>::defaultValue_(), t.defaultValue);
        const typename JsonValueType::ConstMemberIterator &it = jsonValue.FindMember(details::JsonKey<Nexus::Property, typename JsonValueType::Ch>::valid_());
        if (it == jsonValue.MemberEnd()) [[likely]] {
            t.valid = std::nullopt;
        } else {
            t.valid = std::make_optional<std::vector<Nexus::PropertyValue>>();
            ...
            for (const auto &item: it->value.GetArray()) {
                Nexus::PropertyValue propertyValue;
                Nexus::PropertyType::PropertyType type = Codec<decltype(propertyValue)>::template from_json<typename JsonValueType::ValueType>(item, propertyValue);
                if (t.type != type) [[unlikely]] {
                    if (type == Nexus::PropertyType::PropertyType::STRING) {
    ...
    static void to_binary(std::ostream &ostream,
...
#ifdef CHelperDebug
        if (t.type != Nexus::PropertyType::BOOLEAN && t.type != Nexus::PropertyType::STRING && t.type != Nexus::PropertyType::INTEGER) [[unlikely]] {
            throw std::runtime_error("error block state property type");
        }
#endif
        ...
        if (t.valid.has_value()) [[unlikely]] {
            ...
            for (const auto &item: t.valid.value()) {
                Codec<Nexus::PropertyValue>::template to_binary<isNeedConvert>(ostream, item, t.type);
            }
        }
    }
...
CODEC_REGISTER_JSON_KEY(Nexus::BlockPropertyDescription, propertyName, description, values, valueName);

template<>
struct serialization::Codec<Nexus::BlockPropertyDescription> : BaseCodec<Nexus::BlockPropertyDescription> {

    using Type = Nexus::BlockPropertyDescription;
    ...
    static void to_json(typename JsonValueType::AllocatorType &allocator,
                        ...
                        const Type &t) {
        jsonValue.SetObject();
        Codec<decltype(t.propertyName)>::template to_json_member<JsonValueType>(allocator, jsonValue, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::propertyName_(), t.propertyName);
        Codec<decltype(t.description)>::template to_json_member<JsonValueType>(allocator, jsonValue, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::description_(), t.description);
        ...
        for (const auto &item: t.values) {
            ...
            perValue.SetObject();
            Codec<decltype(item.valueName)>::template to_json_member<JsonValueType>(allocator, perValue, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::valueName_(), item.valueName, t.type);
            Codec<decltype(item.description)>::template to_json_member<JsonValueType>(allocator, perValue, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::description_(), item.description);
            values.PushBack(std::move(perValue), allocator);
        }
        jsonValue.AddMember(JsonValueType(details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::values_(), allocator), std::move(values), allocator);
    }
    ...
    static void from_json(const JsonValueType &jsonValue,
        ...
        t.release();
        Codec<decltype(t.propertyName)>::template from_json_member<JsonValueType>(jsonValue, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::propertyName_(), t.propertyName);
        Codec<decltype(t.description)>::template from_json_member<JsonValueType>(jsonValue, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::description_(), t.description);
        bool hasPropertyType = false;
        for (const auto &item: serialization::find_array_member_or_throw(jsonValue, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::values_())) {
            Nexus::BlockPropertyValueDescription blockPropertyValueDescription;
            Nexus::PropertyType::PropertyType type = Codec<decltype(blockPropertyValueDescription.valueName)>::template from_json_member<typename JsonValueType::ValueType>(item, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::valueName_(), blockPropertyValueDescription.valueName);
            if (hasPropertyType) [[unlikely]] {
                if (t.type != type) [[likely]] {
                    if (type == Nexus::PropertyType::STRING) {
            ...
            Codec<decltype(blockPropertyValueDescription.description)>::template from_json_member<typename JsonValueType::ValueType>(item, details::JsonKey<Nexus::BlockPropertyDescription, typename JsonValueType::Ch>::description_(), blockPropertyValueDescription.description);
...
CODEC(Nexus::PerBlockPropertyDescription, blocks, properties)

CODEC(Nexus::BlockPropertyDescriptions, common, block)

CODEC_WITH_PARENT(Nexus::BlockId, Nexus::NamespaceId, properties)

CODEC(Nexus::BlockIds, blockStateValues, blockPropertyDescriptions)
